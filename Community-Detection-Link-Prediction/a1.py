        ...
        for node in graph.neighbors(current):
            if(node2distances[current]<max_depth):
                if node not in visited and node != root:
                    ...
                    visited.append(node)
                    d.append(node)
                if node2distances[node] == node2distances[current] + 1:
                    node2num_paths[node] = node2num_paths[node] + 1
                    node2parents.setdefault(node, []).append(current)
...
def get_subgraph(graph, min_degree):
    ...
    2
    """
    node_list = [node for node in graph.nodes() if graph.degree(node)>=min_degree]
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for node in node2distances:
        if(node[0]!=root):
            if(len(node2parents[node[0]])>1):
                for parent in node2parents[node[0]]:
                    temp[parent] = (temp[node[0]]/len(node2parents[node[0]]))+temp[parent]
                    final[tuple(sorted([node[0],parent]))] = float(temp[node[0]]/len(node2parents[node[0]]))
            else:
                temp[node2parents[node[0]][0]] = temp[node[0]]+temp[node2parents[node[0]][0]]
                final[tuple(sorted([node[0],node2parents[node[0]][0]]))] = float(temp[node[0]])
    return final
    pass
...
def approximate_betweenness(graph, max_depth):
    ...
    for node in graph.nodes():
        ...
        temp = bottom_up(node, node2distances, node2num_paths, node2parents)
        #print(node,sorted(temp.items()))
        for a in temp:
            if a in final:
                final[a] = final[a] + temp[a]
            else:
                final[a] = temp[a]
    for a in final:
        final[a] = final[a] / 2;
...
def partition_girvan_newman(graph, max_depth):
    ...
    edge_list = approximate_betweenness(new_graph, max_depth)
    counter = 0
    while len(components) == 1:
        edge_to_remove = sorted(edge_list.items(), key=lambda x:-x[1])[counter][0]
...
def norm_cut(S, T, graph):
    ...
    """
    #if volume(S,graph)==0:
        #return float(cut(S,T,graph)/volume(T,graph))
    #if volume(T,graph)==0:
        #return float(cut(S,T,graph)/volume(S,graph))
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        ...
        components = sorted(components, key=lambda x: sorted(x.nodes())[0])
        #print(sorted(components[0].nodes()),sorted(components[1].nodes()))
        score.append((depth,norm_cut(components[0].nodes(),components[1].nodes(),graph)))
    return score
    pass
...
def make_training_graph(graph, test_node, n):
    ...
    edges_remove = sorted(temp_graph.edges(test_node))[:n]
    temp_graph.remove_edges_from(edges_remove)
    return temp_graph
    pass
...
def path_score(graph, root, k, beta):
    ...
    for n in graph.nodes():
        if root!=n and not graph.has_edge(root, n):
            scores.append(((root,n),pow(beta,node2distances[n])*node2num_paths[n]))
    #print(sorted(scores, key=lambda x: (-x[1],x[0]))[:k])
    return sorted(scores, key=lambda x: (-x[1],x[0]))[:k]
    pass
...
def evaluate(predicted_edges, graph):
    ...
    0.5
    """
    return len([edge for edge in predicted_edges if graph.has_edge(*edge)]) / len(predicted_edges)
    pass
