...
def get_friends(twitter, screen_name):
    ...
    ###for friend in friends:
       ###request.append(friend)
    request =  [friend for friend in friends]
...
def add_all_friends(twitter, users):
    ...
    for u in range(len(users)):
        users[u]['friends']=get_friends(twitter,users[u]['screen_name'])
    ###TODO
    pass
...
def friend_overlap(users):
    ...
    for u in range(len(users)):
        if(u==3):
            break
        for u2 in range(u+1,len(users)):
            list2.append((users[u]['screen_name'],users[u2]['screen_name'],len(set(users[u]['friends']).intersection(users[u2]['friends']))))
            """
            count=0
            for i in range(len(users[u]['friends'])):
                if(users[u]['friends'][i] in users[u2]['friends']):
                    count=count+1
            list1.append((users[u]['screen_name'],users[u2]['screen_name'],count))    
            """
    return(sorted(list2,key=lambda N:N[2], reverse=True))
    ###TODO
    pass
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    request = robust_request(twitter, "users/lookup", {"user_id" : set(users[2]["friends"]).intersection(users[3]["friends"])})
...
def create_graph(users, friend_counts):
    ...
    G = nx.Graph()
    for u in users:
        G.add_node(u['screen_name'])
        for friends in u['friends']:
            if(friend_counts[friends]>1):
                G.add_node(friends)
                G.add_edge(friends,u['screen_name'])
    return(G)
    ###TODO
    pass
...
def draw_network(graph, users, filename):
    ...
    nx.draw_networkx(graph,node_size=100,edge_color="#0000FF",labels=label)
