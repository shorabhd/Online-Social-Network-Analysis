...
def token_pair_features(tokens, feats, k=3):
    """
    Compute   within a window of size k.
    ...
    for i in range(0,len(tokens)):
        if (i+k <= len(tokens)):
            for t in range(i,i+k):
                if(t+1 < i+k):
                    feats["token_pair="+tokens[t]+"__"+tokens[t+1]]=feats["token_pair="+tokens[t]+"__"+tokens[t+1]]+1
            feats["token_pair="+tokens[i]+"__"+tokens[t]]=feats["token_pair="+tokens[i]+"__"+tokens[t]]+1
    """
    comb=[]
    for i in range(0,len(tokens)):
        if (i+k <= len(tokens)):
            comb.extend(list(combinations(tokens[i:i+k],2)))
    for c in comb:
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    #print(vocab.items())
    for token in tokens_list:
        for t in sorted(feat[rows]):
            if(t[0] in vocab.keys()):
                row.append(rows)
                col.append(vocab[t[0]])
                data.append(t[1])
        rows+=1
    #print(len(row),len(col),len(data),len(vocab))
    X = csr_matrix((data,(row,col)),dtype=np.int64)
    """
    remove_cols = []
    #print(X.getnnz(axis=0))
    for i,col in enumerate(X.getnnz(axis=0)):
        if col < min_freq:
    ...
    for v in vocab.items():
        if v[1] not in remove_cols:
            new_vocab[v[0]] = j
            j+=1
    print("OLD:",len(vocab),"NEW:",len(new_vocab),"OG X:",X.A.shape,"M X:",X[:,X.getnnz(axis=0)>=min_freq].shape)        
    """
    return X,vocab
    #return X[:, X.getnnz(axis=0) >= min_freq],vocab
    pass
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    for L in range(1,len(feature_fns)+1):
        for sub in combinations(feature_fns,L):
            features.append(sub)
    ...
    for punct_val in punct_vals:
        ...
        for min_freq in min_freqs:
            for feature in features:
                ...
                final=dict()
                final['punct'] = punct_val
                final['features'] = feature
                final['min_freq'] = min_freq
                final['accuracy'] = cross_validation_accuracy(clf, X, labels, 5)
                f.append(final)
    return sorted(f,key=lambda x:-x['accuracy'])
    pass
...
def mean_accuracy_per_setting(results):
    ...
    for setting in set(settings):
        ...
        if(setting[0]=='features'):
            f=[]
            f.append(setting[0]+"=")
            for i in setting[1]:
                f.append(i.__name__)
            s = " ".join(f)
            res.append((np.mean(acc),s))
        else: 
            res.append((np.mean(acc),(setting[0] + "=" + str(setting[1]))))
...
def top_coefs(clf, label, n, vocab):
    ...
    pos=[]
    neg=[]
    for k,v in zip(sorted(vocab.keys()), clf.coef_[0]):
        if v>=0:
            pos.append((k,v))
        else:
            neg.append((k,abs(v))) 
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for i in range(0,X_test.shape[0]):
        if predictions[i] != test_labels[i]:
            p[i] = proba[i,predictions[i]]
    for x in sorted(p.items(), key=lambda x:-x[1])[:n]:        
        print("truth=",test_labels[x[0]]," predicted=",predictions[x[0]]," proba=",x[1])
        print(test_docs[x[0]])
    pass
